...
export const normaliseCartData = (cartData: CartItem[], table: string | null , restaurantId: string) =>{
    const cart = cartData.map((item: CartItem)=>({itemId: item.itemId, name:item.name, basePrice: item.basePrice, quantity: item.quantity, specialInstructions: item.specialInstructions,
        ...
        return {cart, table, restaurantId}
}